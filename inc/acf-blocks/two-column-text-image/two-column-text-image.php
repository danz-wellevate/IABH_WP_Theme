...
$class_name = 'block--custom-layout__two-column-text-image';
if ( ! empty( $block['className'] ) ) {
    $class_name .= ' ' . $block['className'];
}

// ACF fields
$bgColor = get_field('two-column-text-image_background_color');
...
// Image size radio button field
// Choices in ACF: "Default" and "Change"
// When "Change" is selected, tablet = 584x631 and mobile = 260x481
$image_size = get_field('image_size'); // returns the value: 'Default' or 'Change'
$image_size_class = ( $image_size === 'Change' ) ? 'image-size--change' : 'image-size--default';

// True/False: should the image bleed to the edge of the screen?
$image_bleed = get_field('image_bleed'); // true or false
$bleed_class = $image_bleed ? 'image-bleed--on' : 'image-bleed--off';

// True/False: should the image be centered on tablet/mobile?
$center_on_mobile = get_field('center_on_mobile'); // true or false
$center_class = $center_on_mobile ? 'image-center--on' : 'image-center--off';
...
<div class="block--custom-layout <?= $class_name ?>" <?= $anchor_id ?> style="background-color: <?= esc_attr($bgColor) ?>;">
  <div class="container">
    <!-- Block Content -->
    <div class="two-column__wrapper image-<?= esc_attr($getPos['value']) ?> <?= esc_attr($bleed_class) ?> <?= esc_attr($center_class) ?>">
      ...
      <!-- Add the $image_size_class to the image content wrapper -->
      <div class="two-column__image-content <?= esc_attr($image_size_class) ?>">
